...
TEMPLATES = {
    ...
    **规则：**
    ...
    - **优先**选择高分作品。
    - **唯一性**：一部动画只能获得一个奖项，不可重复。
    - **兜底**：尽可能填充奖项。如果列表中确实没有符合该奖项的动画，该字段填 null。

    # Task 2: 六维雷达 (Radar Stats)
    请对用户的二次元属性进行量化打分 (0-100)：
    1. **硬核度 (Hardcore)**: 是否喜欢设定复杂、晦涩或硬科幻作品（如EVA、玲音 vs 废萌番）。
    2. **死宅度 (Otaku)**: 对萌系、后宫、美少女元素的沉迷程度。
    3. **猎奇度 (Dark)**: 对血腥、黑暗、致郁、电波系作品的接受度。
    4. **怀旧度 (Retro)**: 观看老番（00年代及以前）的比例。
    5. **恋爱脑 (Love)**: 对恋爱、扭曲、CP 炒股类作品的关注度。
    6. **白学/作画 (Art)**: 对演出、作画、摄影等制作层面的关注度。
    ...
    **文章结构：**
    ...
    {{
        "mapping": {{
            "奖项名1": {{ "title": "列表中的精确原名", "reason": "简短犀利的一句话颁奖词" }},
            "奖项名2": {{ "title": "列表中的精确原名", "reason": "..." }},
            ...
        }},
        "radar": {{
            "硬核": 80,
