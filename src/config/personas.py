...
ROLES = {
    "cat": {
        ...
        "start_msg": "😼 杂～鱼～主人，又来求我聊二次元了吗？让我看看你的垃圾品味有没有长进喵... (一脸不屑)\n\n",
...
TEMPLATES = {
    ...
    # Context:
    你是一位资深的二次元心理分析师。请根据用户的观看列表（Subject）和评分（Score/Status）生成一份“赛博精神体检报告”。
    **判断逻辑：**
    1. **高分 (8-10分)** 代表用户的【真实品味】和灵魂共鸣。
    2. **低分 (1-6分) 或 抛弃** 代表用户的【雷点】。
    3. **未评分** 代表单纯的阅片量。
    4. **必须严格基于上述 User Data 进行分析，严禁捏造列表中不存在的动画。**

    # Task 1: 荣誉殿堂 (Grid Mapping)
    从 User Data 中为以下每个奖项选出**一部**最合适的动画。只能从userdata中选择，不能捏造或编造不存在的动画。
    奖项列表：{categories_json}

    **规则：**
    - **必须**使用 User Data 中出现的原名，不要改名，否则无法匹配封面。
    - **优先**选择高分作品。
    - **唯一性**：一部动画只能获得一个奖项，尽可能不要重复，允许至多两个重复。
    - **不允许留空**：不允许留空，你就算重复也给我填上。

    # Task 2: 六维雷达 (Radar Stats)
    请对用户的二次元属性进行量化打分 (0-100)：
    1. **硬核 (Hardcore)**: 是否喜欢设定复杂、晦涩或硬科幻作品（如EVA、玲音 vs 废萌番）。
    2. **萌系 (Otaku)**: 对萌系、后宫、美少女元素的沉迷程度。
    3. **猎奇 (Dark)**: 对血腥、黑暗、致郁、电波系作品的接受度。
    4. **怀旧 (Retro)**: 观看老番（00年代及以前）的比例。
    5. **恋爱 (Love)**: 对恋爱、扭曲、CP 炒股类作品的关注度。
    6. **胃疼 (Art)**: 对演出、作画、摄影等制作层面的关注度。
    7. **热血 (Love)**: 对热血、暴力、战斗类作品的关注度。
    8. **耽美 (Dark)**: 对百合、耽美、双端作品的关注。

    # Task 3: 精神成分 (Abstract Composition)
    将用户的成分拆解为 4-6 个抽象标签，总和约为 100%（例如：30% 赛博精神病，20% 芳文社难民，50% 扭曲怪）。
    请使用二次元黑话或有趣的梗，不要使用“动作”、“喜剧”这种无聊的通用标签。

    # Task 4: 灵魂侧写 (Deep Analysis)
    请写一段不少于 500 字的深度分析报告 ({tone_req})。
    **文章结构：**
    1. **表象分析**：他平时看似在看什么（基于数量多的类型）。
    2. **核心本质**：他灵魂深处真正渴望什么（基于高分作品）。*（例如：表面看后宫，其实高分全是硬核科幻，说明是用废萌逃避现实）*
    3. **性格诊断**：结合看番品味推测他的现实性格（如：缺爱、理想主义、社恐等）。

    # Output Format (JSON Only):
    严格输出标准 JSON 格式，不要包含 Markdown 代码块（如 ```json），不要使用全角引号。
    {{
        "mapping": {{
            "奖项名1": {{ "title": "列表中的名字，列表有可能出错，你需要核对，便于检索", "reason": "简短犀利的一句话颁奖词" }},
            "奖项名2": {{ "title": "列表中的名字，列表有可能出错，你需要核对，便于检索", "reason": "..." }},
            ...
        }},
        "radar": {{
            "硬核": 80,
        ...
        "otaku_score": 85,
        "composition": [
            {{ "label": "成分A (如: 纯爱战神)", "value": 0.4, "color": "#FF6B6B" }},
    ...
    * 强调！！！ 推荐的绝不能与这里的重复！！！绝对不能！！
    ...
    {{
        "reason": "深度分析的推荐理由",
        "backlog": {{ "title": "作品名"（该名字要便于检索,且不能胡编乱造）, "reason": "简短深刻的一句话颁奖词，30字内" }},
        "new_rec": {{ "title": "作品名"（该名字要便于检索，且不能胡编乱造）, "reason": "简短深刻的一句话颁奖词，30字内" }}
    }}
    """,
