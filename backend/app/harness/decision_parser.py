...
class DecisionParseError(ValueError):
    ...
    def __init__(self, error_code: ErrorCode, message: str = "Invalid model decision") -> None:
    ...
    def parse(
        ...
        *,
        ...
        if not isinstance(result, ModelCallResult):
            raise DecisionParseError(ErrorCode.INVALID_REQUEST)
        ...
        if payload.get("schema_version", self.schema_version) != self.schema_version:
            raise DecisionParseError(ErrorCode.INVALID_REQUEST, "Unknown decision schema version")
        ...
        try:
            ...
            raise DecisionParseError(ErrorCode.INVALID_REQUEST) from exc
        ...
        if decision.action == "invoke":
            ...
            if decision.capability_version != self.schema_version:
                raise DecisionParseError(ErrorCode.INVALID_REQUEST, "Unknown capability contract version")
            ...
            if len(encoded) > self.max_arguments_bytes:
                raise DecisionParseError(ErrorCode.INVALID_REQUEST, "Decision arguments exceed the public limit")
        return decision
    ...
    def _payload(self, result: ModelCallResult) -> dict[str, Any]:
        ...
        if result.tool_calls:
            if len(result.tool_calls) != 1:
                raise DecisionParseError(ErrorCode.INVALID_REQUEST, "Exactly one tool proposal is required")
