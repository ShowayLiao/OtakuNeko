...
def test_edge_cases():
    ...
    print(f"  有效评分数量：{profile.get('llm_summary', {}).get('total_rated', 0)}")
    print(f"  标签数量：{len(profile.get('llm_summary', {}).get('taste_dictionary', {}))}")
    ...
    profile = generate_user_profile(invalid_data)
    print(f"  有效评分数量：{profile.get('llm_summary', {}).get('total_rated', 0)}")
    print(f"  标签数量：{len(profile.get('llm_summary', {}).get('taste_dictionary', {}))}")
    ...
    profile = generate_user_profile(small_sample)
    print(f"  有效评分数量：{profile.get('llm_summary', {}).get('total_rated', 0)}")
    print(f"  标签数量：{len(profile.get('llm_summary', {}).get('taste_dictionary', {}))}")
