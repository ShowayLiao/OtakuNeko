            ...
            {isPopoverOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden z-50">
                ...
                <div className="p-3 border-t border-gray-100">
                  <button
                    className={`w-full py-2 px-3 bg-[#FFB347] text-[#8B4513] font-medium rounded-lg shadow-sm text-sm hover:bg-[#FF8C00] hover:text-white transition-colors ${isSyncing ? 'opacity-70 cursor-not-allowed' : ''}`}
                    onClick={async () => {
                      await handleSync();
                      setIsPopoverOpen(false);
                    }}
