/* eslint-disable @typescript-eslint/no-explicit-any -- cell data supports multiple schedule sources. */
...
const TimelineCell: React.FC<TimelineCellProps> = ({
  ...
  return (
    ...
    >
      <div className="h-full w-full p-[1px] relative overflow-hidden" style={{ boxSizing: 'border-box' }}>
        {items.length > 0 && (
          items.length > 1 ? (
            ...
            items.map((item) => (
              <DraggableItemWrapper
                ...
                id={`board-${item.subject.source}-${item.subject.source_id}`}
                data={item}
              >
                <div style={{ height: '100%', width: '100%', minHeight: 0 }}>
                  <TimelineMediaCard
...
const arePropsEqual = (prevProps: TimelineCellProps, nextProps: TimelineCellProps): boolean => {
  ...
  const detailHandlerEqual = prevProps.onOpenDetail === nextProps.onOpenDetail;
  if (!prevProps.isOver && !nextProps.isOver) {
    return itemsAreEqual() && slotHeightEqual && detailHandlerEqual;
  }
  ...
  const nextDragItemKey = nextProps.activeDragItem?.subject?.source && nextProps.activeDragItem?.subject?.source_id
    ...
    : null;
  return isOverEqual && itemsAreEqual() && prevDragItemKey === nextDragItemKey && slotHeightEqual && detailHandlerEqual;
};
