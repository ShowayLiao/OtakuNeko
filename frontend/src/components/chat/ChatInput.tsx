...
export const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (message.trim()) {
      ...
      // 如果有引用上下文，将其信息拼接到消息中
      if (referenceItem) {
        const score = referenceItem.score ?? referenceItem.rating_details?.score ?? 0;
        const scoreNumber = typeof score === 'number' ? score : parseFloat(String(score));
        const contextText = `[Context: {"name": "${referenceItem.name_cn || referenceItem.name}", "score": ${scoreNumber.toFixed(1)}}]\n\n`;
