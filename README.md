...
<p>
    <b>OtakuNeko 不仅仅是一个个人助手。</b><br>
    它能同步<b>指定用户名</b>的 Bangumi 收藏，利用 AI 对你的二次元成分进行充分鉴定，<br>
...
<div align="center">
    <img src="asset/profile2.png" width="800" alt="Dashboard Preview">
    <br>
    <i>OtakuNeko 生成用户画像</i>
</div>

<div align="center">
    <img src="asset/profile_grid2.png" width="400" alt="Dashboard Preview">
    <br>
...
## 📅 更新日志
### v1.3 (2025-12-19)（only 云端）
- 😭上线持续更新，太肝了，后续没问题估计过一阵子再推出一个大版本吧。
- ✨ **新增**：
    * 针对部分~~老资历~~用户看过动画太多，容易导致token爆炸的问题，新增了token优化模式，打开后，只输入高分top100和低分top50进行成分分析。
    * 年度报告新增雷达图和饼图，优化了提示词。
    * 为了在Streamlit免费服务器提高稳定性，新增了排队保护模式，高峰期需要排队等待；同时，长时间不操作会自动退出会话，以减少内存占用。
- 🐛 **修复**：
    * 针对云服务的问题，修复了一些bug。
    * 降低了部分模型的温度，以兼容AI服务商。
- 💄 **优化**：
    * 大改云端后端，内存峰值占用从1G下降到了200MB。
    * 将本地的RAG方法改为使用阿里云服务，因此依赖包的体积大幅降低，docker高达10G的情况应该不会出现了。
